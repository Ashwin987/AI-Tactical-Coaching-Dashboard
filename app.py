...
from dotenv import load_dotenv # NEW: For reading local .env file
...
# Pull the API key. In production (Streamlit Cloud), this reads from the Secrets Manager.
master_key = os.getenv("GEMINI_API_KEY")

# Create the API_KEYS list required by the rest of the script
...
* **How it is used:** Coaches use this to identify periods of sustained dominance, pinpoint when momentum shifted, and evaluate the effectiveness of their team's transitional threat relative to their opponent's possession.

**Average Threat Score**
Calculates the mean value of the Threat Score exclusively during the minutes a team held possession. 
* **How it is used:** This acts as an efficiency metric. A team might hold the ball for 60% of the match but have a low Average Threat Score, indicating sterile possession. Conversely, a counter-attacking team might have low possession but a massive Average Threat Score, indicating highly lethal, direct attacks.
...
* **How it is used:** This immediately diagnoses a team's overarching territory. A primary zone in the Defensive Third suggests a team is either intentionally building from the back or being pinned down by an aggressive opponent. A primary zone in the Middle Third indicates a team is dictating the game's tempo, engaging in a physical midfield battle, or holding sterile possession while struggling to penetrate a low block. A primary zone in the Attacking Third signifies sustained offensive pressure, successfully trapping the opponent deep in their own half, and controlling the game entirely on the front foot.
...
* **How it is used:** This is a purer measure of offensive pressure than simple possession statistics. It tells a coach exactly how long they successfully kept the opponent under siege.
...
* **How it is used:** Coaches use this to visualize where the battle was actually fought. If a team's heatmap is entirely blue in the middle third, they controlled the midfield but struggled to penetrate the final third.

**Defensive Block Height (Positioning)**
Measures the *starting position* of a team's defensive wall out of possession:
* **Low Block:** The team 'parked the bus,' retreating deep into their own penalty area to defend.
* **Mid Block:** The team held their defensive line near the midfield circle, staying compact.
* **High Block:** The team pushed their defenders aggressively high up the pitch into the opponent's half.
* **How it is used:** Coaches analyze this alongside Pressing Intensity to determine if their out-of-possession structure was executed correctly, or if they were forced deeper than intended by the opponent's build-up play.

**Pressing Intensity (1-10 Scale)**
An AI-calculated metric measuring the aggression of the team's tackling and closing down. (1-3: Passive/Defending the box, 4-6: Engaging near halfway line, 7-10: Aggressive/Hunting the ball).
* **How it is used:** Allows managers to quantify their team's work rate off the ball and verify if pressing triggers (like a poor touch or backward pass) were actually acted upon with the intended intensity.
...
* **How it is used:** Coaches use this to identify attacking over-reliance on a specific player (e.g., forcing everything down the left flank), analyze if their wingers are being isolated, or map out where the opponent's defensive structure is most vulnerable.

---
### 🧠 The Backend Architecture & Prompt-Based Fine-Tuning
This dashboard is powered by a custom multimodal data pipeline that converts raw match footage into structured, mathematical tactical arrays. Standard out-of-the-box AI models fail at this task due to spatial confusion and context-window limitations. To solve this, the engine relies on heavy architectural constraints and strict in-context fine-tuning applied to Google's **Gemini 2.5 Flash** vision-language model.

**1. Local Micro-Batching Protocol**
To optimize API costs while ensuring 100% mathematical accuracy, the engine utilizes a Local Micro-Batching Protocol. The Python backend physically slices the match into strict 1-minute intervals locally, uploading and processing them sequentially. This guarantees the AI maintains "tunnel vision" on specific tactical actions, entirely preventing the hallucination fatigue that plagues LLMs processing long-form video.

**2. Architectural Constraint Fine-Tuning**
Rather than relying on traditional weight-updating, the model is strictly fine-tuned through systemic prompt architecture and logic locks:
* **The High-Value Override:** A minute strictly belongs to the team that held the ball for the majority of the 60 seconds. However, if the defending team steals the ball and registers a clear goal-scoring threat or shot, they "steal" possession for that minute and receive the threat points.
* **The Anti-Illusion Protocol:** Vision models naturally suffer from a "center-frame bias" caused by broadcast camera panning. I engineered a prompt constraint that strictly commands the AI to index its spatial mapping against the pitch's physical touchlines and penalty boxes, preventing the model from falsely logging wide flank attacks as central attacks.
* **Dual-Logic Integration:** The dashboard decouples duration from frequency. "Time in Attack" is evaluated using estimated fractional minutes, while "Attacking Bias" is generated by counting the pure frequency of distinct attacking sequences.
...
I have extensive experience designing end-to-end data systems, from building predictive machine learning models and automating data workflows to deploying full-stack applications. I enjoy translating complex data into actionable insights to drive real-world decision-making. 

Beyond research, I am an aspiring sports analyst focused on tactical analysis, combining domain knowledge with advanced analytics to evaluate performance and risk. 

🔗 [Connect with me on LinkedIn](https://www.linkedin.com/in/ashwin-ramaseshan-a63188201/)
"""

# --- NEW: PDF Download Helper Function ---
def get_pdf_download_button():
    # UPDATED: Relative path for Streamlit Cloud deployment
...
if st.session_state.step == 1:
    ...
    if extract_btn:
        # Check if the key exists either in the local .env or the Streamlit Secrets
        if not valid_keys:
            st.error("Please add your Gemini API Key to the .env file (local) or Secrets Manager (Streamlit Cloud).")
            ...
            # --- NEW: Mini-Game Carousel HTML/JS with Quotes separated ---
            carousel_html = """
            <div style="font-family: sans-serif; display: flex; flex-direction: column; align-items: center;">
                <div style="width: 100%; position: relative; border-radius: 15px; border: 3px solid #b2dfdb; overflow: hidden; box-shadow: inset 0px 0px 15px rgba(0,0,0,0.1); margin-bottom: 20px;">
                    ...
                    <div id="game2" class="game-view" style="display: none; width: 100%; height: 350px; background: linear-gradient(to bottom, #2e7d32, #4db6ac); position: relative; flex-direction: column; align-items: center; overflow: hidden;">
                        <h2 style="color: #ffffff; margin-top: 30px; margin-bottom: 5px; user-select: none; text-shadow: 1px 1px 2px rgba(0,0,0,0.5);">London 2012 Goalkeeper</h2>
            ...
            <script>
                ...
                updateQuote();
                setInterval(updateQuote, 10000); // 10 second loop
                ...
                // --- GAME 2 LOGIC (London 2012 Goalkeeper) ---
                const goalie = document.getElementById('goalie');
                const ball2 = document.getElementById('ball2');
                const score2 = document.getElementById('score2');
                let gX = 50; // percentage
                let b2X = 50, b2Y = -50; // starting ball coords
                let b2Vy = 3.5;
                let s2 = 0;
                
                function resetBall2() {
                    b2Y = -50;
                    b2X = 20 + Math.random() * 60; // random between 20% and 80%
                    b2Vy = 3.5 + (s2 * 0.3); // Gets slightly faster with every save!
                }
                
                function loop2() {
                    b2Y += b2Vy;
                    
                    if (b2Y > 260) { // Goal line
                        if (Math.abs(b2X - gX) < 12) { // Saved (hitbox)
                            s2++;
                ...
                document.addEventListener('keydown', function(event) {
                    ...
                    if (currentGame === 2) {
                        if (event.code === 'ArrowLeft') { 
                            event.preventDefault();
                            gX -= 7; 
                            if(gX < 15) gX = 15; // Left bounds
                            goalie.style.left = gX + '%'; 
                        }
                        if (event.code === 'ArrowRight') { 
                            event.preventDefault();
                            gX += 7; 
                            if(gX > 85) gX = 85; // Right bounds
                            goalie.style.left = gX + '%'; 
                        }
                    }
                });
            ...
            try:
                ...
                # --- UPGRADE A: MULTITHREADING BATCH EXECUTION ---
                if TOTAL_MINUTES > 1:
                    max_workers = 3 
                    completed_count = 1 
